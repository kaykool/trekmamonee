<script lang="ts">
	...
	$effect(() => {
		...
		const sub = liveQuery(() =>
			Promise.all([getMonthSummary(year, month), getCategoryBreakdown(year, month, 'expense')])
		).subscribe(([s, b]) => {
			summary = s;
			breakdown = b;
		});
...
<div class="flex flex-col gap-6 pb-6">
	<!-- Month Selector -->
	<div
		class="flex items-center justify-between rounded-2xl bg-surface-light p-2 shadow-sm dark:bg-surface-dark"
	>
		<button
		...
		<span class="text-base font-semibold capitalize text-text-light dark:text-text-dark"
			>{displayMonth}</span
		>
		<button
			class="flex h-10 w-10 items-center justify-center rounded-xl transition-colors hover:bg-surface-dark/5 text-text-light/60 hover:text-text-light dark:hover:bg-surface-light/10 dark:text-text-dark/60 dark:hover:text-text-dark"
