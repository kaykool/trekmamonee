...
<div class="flex flex-col gap-6">
  {#if isLoading}
    <div class="flex h-40 items-center justify-center">
      <div class="h-8 w-8 animate-spin rounded-full border-4 border-surface-dark/20 border-t-primary dark:border-surface-light/20"></div>
    </div>
  {:else}
    <!-- Type toggle -->
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <div class="grid grid-cols-4 gap-2">
          {#each categories as category}
            <button 
              class="flex aspect-square flex-col items-center justify-center gap-1 rounded-md p-2 transition-colors {selectedCategoryId === category.id ? 'bg-bg-dark/10 ring-2 ring-primary dark:bg-surface-light/20' : 'bg-surface-dark/5 hover:bg-surface-dark/10 dark:bg-surface-light/5 dark:hover:bg-surface-light/10'}"
              onclick={() => selectedCategoryId = category.id}
            >
              <div class="flex h-8 w-8 items-center justify-center rounded-full {category.color} text-white shadow-sm">
                {category.icon}
              </div>
              <span class="truncate text-[10px] text-text-light/80 dark:text-text-dark/80 max-w-full">{category.name}</span>
            </button>
          {/each}
        </div>
      </div>
    ...
    <div class="flex gap-3">
      <Button variant="secondary" class="flex-1" onclick={() => goto('/transactions')} disabled={isSaving}>Cancel</Button>
      <Button 
