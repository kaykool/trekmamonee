<script lang="ts">
  ...
  $effect(() => {
    ...
    const observable = liveQuery(() => 
      db.categories.where('type').equals(currentType).toArray()
    );
...
<div class="flex flex-col gap-2">
  <div class="flex items-center justify-between">
    ...
    <button 
      ...
      onclick={(e) => { e.preventDefault(); isEditingCategories = !isEditingCategories; editingCategoryId = null; }}
    ...
    >
      ...
      {:else}
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>
      {/if}
    </button>
  </div>
  <div class="grid grid-cols-4 gap-3 content-start">
    {#each categories as category}
      <div 
        role="button"
        tabindex="0"
        class="group relative flex flex-col items-center justify-center gap-2 rounded-xl p-3 transition-all {selectedCategoryId === category.id && !isEditingCategories ? 'bg-background dark:bg-background-dark shadow-sm ring-2 ring-primary/50' : 'bg-surface-dark/5 hover:bg-surface-dark/10 dark:bg-surface-light/5 dark:hover:bg-surface-light/10'} cursor-pointer"
        onclick={(e) => {
          e.preventDefault();
          if (isEditingCategories) {
      ...
      >
        {#if isEditingCategories}
          <button
            ...
            onclick={(e) => { e.stopPropagation(); e.preventDefault(); promptDeleteCategory(category); }}
            aria-label="Delete category"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
          </button>
        {/if}
        <div class="relative flex h-12 w-12 items-center justify-center rounded-full {category.color} text-2xl text-white shadow-sm transition-transform {selectedCategoryId === category.id && !isEditingCategories ? 'scale-110' : ''}">
          {category.icon}
          {#if isEditingCategories}
            <div class="absolute inset-0 flex items-center justify-center rounded-full bg-black/40 opacity-0 transition-opacity group-hover:opacity-100">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>
            </div>
          {/if}
        </div>
        ...
        {#if editingCategoryId === category.id}
          ...
          />
        {:else}
          <span class="w-full truncate text-center text-xs font-medium {selectedCategoryId === category.id && !isEditingCategories ? 'text-primary' : 'text-text-light/80 dark:text-text-dark/80'}">{category.name}</span>
        {/if}
      </div>
    {/each}
    ...
    {#if isEditingCategories}
      <button 
        ...
        onclick={(e) => { e.preventDefault(); addNewCategory(); }}
      >
        <div class="flex h-12 w-12 items-center justify-center rounded-full bg-surface-dark/5 dark:bg-surface-light/5 text-2xl text-text-light/50 dark:text-text-dark/50">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
        </div>
        <span class="w-full truncate text-center text-xs font-medium text-text-light/60 dark:text-text-dark/60">New</span>
      </button>
    {/if}
  </div>
</div>
