...
>
	<div class="flex items-center gap-3">
		<!-- Category Icon -->
		<div
			class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full {category?.color ||
				'bg-gray-500'} text-xl text-white shadow-sm"
		>
			{category?.icon || '🏷️'}
		</div>
		...
		<div class="flex flex-col items-start">
			<span
				class="font-semibold text-text-light dark:text-text-dark max-w-[150px] truncate text-left"
				>{transaction.itemName || 'Unknown Item'}</span
			>
			<span
				class="truncate text-xs text-text-light/60 dark:text-text-dark/60 max-w-[150px] text-left"
				>{category?.name || 'Unknown Category'}</span
			>
		</div>
	</div>
