...
{#if mode === 'verify' && pinStore.hasPin && pinStore.isLocked}
  <div class="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-background-light dark:bg-background-dark" in:fade>
    <div class="flex flex-col items-center gap-8 w-full max-w-sm mx-auto px-6">
      
      <div class="flex flex-col items-center gap-1">
        <div class="p-3 bg-surface-dark/5 dark:bg-surface-light/10 rounded-2xl mb-2 text-primary-light dark:text-primary-dark">
      ...
      <div class="flex gap-4 mb-4">
        {#each Array(MAX_LENGTH) as _, i}
          <div class="h-4 w-4 rounded-full border-2 transition-colors {input.length > i ? 'bg-primary-light border-primary-light dark:bg-primary-dark dark:border-primary-dark' : 'border-surface-dark/20 dark:border-surface-light/20'}"
      ...
      <div class="grid grid-cols-3 gap-3 w-full select-none">
        {#each [1, 2, 3, 4, 5, 6, 7, 8, 9] as num}
          <button class="w-full h-14 rounded-xl bg-surface-light dark:bg-surface-dark shadow-sm text-xl font-medium active:scale-95 transition-transform" onclick={() => handleInput(num.toString())}>
            {num}
          </button>
        {/each}
        <div></div>
        <button class="w-full h-14 rounded-xl bg-surface-light dark:bg-surface-dark shadow-sm text-xl font-medium active:scale-95 transition-transform" onclick={() => handleInput('0')}>
          0
        </button>
        <button aria-label="Backspace" class="w-full h-14 rounded-xl flex items-center justify-center text-text-light dark:text-text-dark active:scale-95 transition-transform" onclick={handleBackspace}>
  ...
  <div class="flex flex-col items-center gap-6 w-full max-w-sm mx-auto">
    <div class="flex flex-col items-center gap-1">
      <h2 class="text-lg font-semibold text-text-light dark:text-text-dark text-center">
        {mode === 'remove' ? 'Enter current PIN' : isConfirming ? 'Confirm new PIN' : 'Create 6-digit PIN'}
      </h2>
    </div>
    ...
    <div class="grid grid-cols-3 gap-3 w-full select-none">
      ...
      {/each}
      <button class="w-full h-14 rounded-xl text-sm font-medium text-text-light/60 dark:text-text-dark/60 active:scale-95 transition-transform" onclick={oncancel}>
        Cancel
      </button>
      <button class="w-full h-14 rounded-xl bg-surface-light dark:bg-surface-dark shadow-sm text-xl font-medium active:scale-95 transition-transform" onclick={() => handleInput('0')}>
        0
