...
<div class="fixed top-4 left-0 right-0 z-[100] flex flex-col items-center gap-2 px-4 pointer-events-none">
  {#each uiState.toasts as toast (toast.id)}
    <div
      animate:flip={{ duration: 300 }}
      in:fly={{ y: -20, duration: 300 }}
      out:fade={{ duration: 200 }}
      class="pointer-events-auto flex w-max max-w-full items-center gap-3 rounded-xl p-3 shadow-lg {typeColors[toast.type]}"
    >
