...
<nav class="fixed bottom-0 left-0 z-40 flex h-16 w-full items-center justify-around border-t border-surface-dark/10 dark:border-surface-light/10 bg-surface-light dark:bg-surface-dark px-2 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
  {#each navItems as item}
    {@const isActive = page.url.pathname === item.path || (item.path !== '/' && page.url.pathname.startsWith(item.path))}
    <button
      class="flex flex-1 flex-col items-center justify-center gap-1 py-1 transition-colors {isActive ? 'text-primary' : 'text-text-light/60 dark:text-text-dark/60 hover:text-text-light dark:hover:text-text-dark'}"
